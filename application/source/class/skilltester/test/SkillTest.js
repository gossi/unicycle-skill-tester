qx.Class.define("skilltester.test.SkillTest", {
	...
	members: {
		...
		_calculateScore: function() {
			...
			var score = mistakes.fatal.length == 0 ? 25 : 0;
			this._distribution = {};
			actions.forEach(function(action) {
				var feedback = this._getFeedback(action.getId());
				var max;
		...
		getAllFeedback: function() {
			...
			actions.forEach(function(action) {
				var feedback = this._getFeedback(action.getId());
				var value = this.getParams()[action.getId()];
				switch (action.getType()) {
				case "boolean":
					var key = value ? 1 : 0;
					if ('values' in feedback && key in feedback['values']) {
						all[action.getId()] = feedback['values'][key];
					}
					break;
					
				case "int":
					if ('values' in feedback) {
						var range = this._getRangeObject(value, feedback['values']);
						all[action.getId()] = range['feedback'];
					}
					break;
					
				case "radio":
					if ('values' in feedback && value in feedback['values']
							&& 'feedback' in feedback['values'][value]) {
						all[action.getId()] = feedback['values'][value]['feedback'];
					}
					break;
				}
			}, this);
